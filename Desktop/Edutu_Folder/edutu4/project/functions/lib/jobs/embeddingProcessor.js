...
exports.refreshEmbeddings = exports.embeddingMaintenanceJob = exports.processEmbeddingBacklog = exports.processUserContextEmbedding = exports.processRoadmapEmbedding = exports.processScholarshipEmbedding = void 0;
 ...
 */
exports.processScholarshipEmbedding = functions.firestore
    ...
    try {
        ...
        await vectorStore_1.vectorStore.storeScholarshipEmbedding(scholarshipId, scholarshipData.title || '', content, {
 ...
 * Automatically generates and stores embeddings
 */
exports.processRoadmapEmbedding = functions.firestore
    ...
    try {
        ...
        // Generate and store embedding
        await vectorStore_1.vectorStore.storeRoadmapEmbedding(roadmapId, title, description, phases, {
 ...
 */
exports.processUserContextEmbedding = functions.firestore
    ...
    try {
        ...
        await enhancedEmbeddingService_1.enhancedEmbeddingService.updateUserContextEmbedding(userId);
        // Log successful processing
 ...
 * Runs every 6 hours
 */
exports.processEmbeddingBacklog = functions.pubsub
 ...
 */
exports.embeddingMaintenanceJob = functions.pubsub
 ...
 */
exports.refreshEmbeddings = functions.https.onCall(async (data, context) => {
...
// Helper Functions
async function removeScholarshipEmbedding(scholarshipId) {
    try {
        const { error } = await vectorStore_1.vectorStore.supabase
...
async function removeRoadmapEmbedding(roadmapId) {
    try {
        const { error } = await vectorStore_1.vectorStore.supabase
...
function isSignificantUpdate(beforeData, afterData) {
...
function hasPreferencesChanged(beforeData, afterData) {
...
function buildScholarshipContent(scholarshipData) {
    const parts = [
...
async function processUnprocessedScholarships() {
    ...
    try {
        ...
        const { data: existingEmbeddings } = await vectorStore_1.vectorStore.supabase
            .from('scholarships_embeddings')
            .select('scholarship_id, content_hash');
        const existingIds = new Set((existingEmbeddings === null || existingEmbeddings === void 0 ? void 0 : existingEmbeddings.map(e => e.scholarship_id)) || []);
        let processedCount = 0;
...
async function processUnprocessedRoadmaps() {
    ...
    try {
        ...
        const { data: existingEmbeddings } = await vectorStore_1.vectorStore.supabase
            .from('roadmap_embeddings')
            .select('roadmap_id');
        const existingIds = new Set((existingEmbeddings === null || existingEmbeddings === void 0 ? void 0 : existingEmbeddings.map(e => e.roadmap_id)) || []);
        let processedCount = 0;
        for (const doc of roadmapsSnapshot.docs) {
            ...
            try {
                await vectorStore_1.vectorStore.storeRoadmapEmbedding(roadmapId, roadmapData.title || '', roadmapData.description || '', roadmapData.phases || roadmapData.steps || [], {
                    skills: roadmapData.skills || [],
                    difficulty: roadmapData.difficulty || 'intermediate',
                    duration: roadmapData.duration || roadmapData.timeline || '6 weeks',
                    userId: roadmapData.userId
...
async function updateStaleUserContexts() {
    ...
    try {
        ...
        const { data: staleContexts } = await vectorStore_1.vectorStore.supabase
        ...
        for (const context of staleContexts || []) {
            try {
                await enhancedEmbeddingService_1.enhancedEmbeddingService.updateUserContextEmbedding(context.user_id);
                updatedCount++;
                // Rate limiting
...
async function cleanupOldEmbeddings() {
    ...
    try {
        ...
        const { data: orphanedScholarships } = await vectorStore_1.vectorStore.supabase
            .rpc('cleanup_orphaned_scholarship_embeddings');
        // Remove embeddings for deleted roadmaps
        const { data: orphanedRoadmaps } = await vectorStore_1.vectorStore.supabase
            .rpc('cleanup_orphaned_roadmap_embeddings');
        console.log(`Cleaned up ${(orphanedScholarships === null || orphanedScholarships === void 0 ? void 0 : orphanedScholarships.length) || 0} orphaned scholarship embeddings`);
        console.log(`Cleaned up ${(orphanedRoadmaps === null || orphanedRoadmaps === void 0 ? void 0 : orphanedRoadmaps.length) || 0} orphaned roadmap embeddings`);
    }
    catch (error) {
        console.error('Error cleaning up old embeddings:', error);
    }
}
async function generateEmbeddingStatistics() {
    console.log('Generating embedding statistics...');
    try {
        const stats = await vectorStore_1.vectorStore.getVectorStats();
        // Log statistics to Firestore for monitoring
        await db.collection('systemStats').doc('embeddings').set(Object.assign(Object.assign({}, stats), { lastUpdated: admin.firestore.FieldValue.serverTimestamp(), generatedAt: new Date().toISOString() }));
...
async function refreshRecentEmbeddings() {
...
async function refreshScholarshipEmbeddings(ids, force) {
    // Implementation for refreshing scholarship embeddings
    return 0;
}
async function refreshRoadmapEmbeddings(ids, force) {
    // Implementation for refreshing roadmap embeddings
    return 0;
}
async function refreshUserContextEmbeddings(ids, force) {
    // Implementation for refreshing user context embeddings
    return 0;
}
async function logEmbeddingProcessing(type, entityId, status, errorMessage) {
    try {
...
//# sourceMappingURL=embeddingProcessor.js.map