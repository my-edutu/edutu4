 ...
 */
export const simpleChatMinimal = functions.https.onRequest(async (req, res) => {
  // Enable CORS
...
function generateIntelligentResponse(
  ...
  const userName = userContext?.name || userProfile?.name || userProfile?.preferences?.name || 'there';
 ...
 */
function generateRAGResponse(message: string, userName: string, ragContext: any): string {
  ...
  scholarships.forEach((scholarship: any, index: number) => {
    ...
    response += `✨ ${scholarship.summary || scholarship.description?.substring(0, 150) + '...' || 'Great opportunity for your career!'}\n\n`;
  });
 ...
 */
function generateScholarshipResponse(userName: string, userProfile: any): string {
  const interests = userProfile?.preferences?.careerInterests || [];
  const educationLevel = userProfile?.preferences?.educationLevel || 'undergraduate';
 ...
 */
function generateCareerResponse(userName: string, userProfile: any): string {
 ...
 */
function generateLearningResponse(userName: string, userProfile: any): string {
 ...
 */
function generateRoadmapResponse(userName: string, userProfile: any): string {
 ...
 */
function generateGoalResponse(userName: string, userProfile: any): string {
 ...
 */
function generateApplicationResponse(userName: string, userProfile: any): string {
 ...
 */
function generateNetworkingResponse(userName: string, userProfile: any): string {
 ...
 */
function generateWelcomeResponse(userName: string, userProfile: any): string {
  const interests = userProfile?.preferences?.careerInterests || [];
 ...
 */
function generateContextualResponse(message: string, userName: string, userProfile: any): string {
