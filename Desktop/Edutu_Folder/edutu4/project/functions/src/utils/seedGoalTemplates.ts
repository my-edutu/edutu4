...
import { GoalTemplate, GoalMilestone, SubTask, Resource } from '../schemas/goalsSchema';

const db = admin.firestore();

export const GOAL_TEMPLATES: Omit<GoalTemplate, 'id' | 'createdAt' | 'updatedAt'>[] = [
  {
...
export async function seedGoalTemplates(): Promise<void> {
  try {
    ...
    await batch.commit();
    console.log(`Successfully seeded ${GOAL_TEMPLATES.length} goal templates`);
 ...
 */
export const seedTemplatesFunction = async (req: any, res: any) => {
  try {
    ...
    res.json({
      success: true,
      message: `Successfully seeded ${GOAL_TEMPLATES.length} goal templates`,
      templates: GOAL_TEMPLATES.map(t => ({ title: t.title, category: t.category }))
    });
