...
export async function getGoalTemplateById(templateId: string): Promise<GoalTemplate | null> {
  try {
...
export async function searchMarketplaceGoals(params: SearchGoalsRequest): Promise<{
...
export async function getMarketplaceGoalById(goalId: string): Promise<MarketplaceGoal | null> {
  try {
...
export async function getFeaturedMarketplaceGoals(): Promise<MarketplaceGoal[]> {
  try {
    const now = new Date();
...
export async function createUserGoal(userId: string, request: CreateGoalRequest): Promise<UserGoal> {
  try {
    const goalId = `goal_${userId}_${Date.now()}`;
    let goalData: Partial<UserGoal>;
    ...
    if (request.sourceType === 'marketplace' && request.sourceId) {
      ...
      goalData = {
        ...
        tags: template.tags,
        roadmap: template.roadmap.map(milestone => ({
          ...milestone,
          isCompleted: false,
          subtasks: milestone.subtasks.map(subtask => ({
            ...subtask,
    ...
    const userGoal: UserGoal = {
      ...
      ...goalData as Required<typeof goalData>,
      status: 'active',
      progress: 0,
      createdAt: new Date(),
      updatedAt: new Date(),
...
export async function getUserGoals(userId: string, status?: string): Promise<UserGoal[]> {
  try {
...
export async function getUserGoalById(goalId: string): Promise<UserGoal | null> {
  try {
...
): Promise<{ progress: number; goal: UserGoal }> {
  try {
    // Extract userId from goalData or search
    const usersSnapshot = await db.collection('users').get();
    let goalDoc: any = null;
    let userIdFromDoc: string = userId;
    ...
    const goalData = goalDoc.data() as UserGoal;
      ...
      if (milestone.id === request.milestoneId) {
        if (request.subtaskId) {
          ...
          const updatedSubtasks = milestone.subtasks.map(subtask => {
            if (subtask.id === request.subtaskId) {
              return {
                ...
                completedAt: request.isCompleted ? new Date() : undefined
              };
            }
            return subtask;
          });
          ...
          return {
            ...
            completedAt: allSubtasksCompleted ? new Date() : milestone.completedAt
          };
        } else {
    ...
    const completedSubtasks = updatedRoadmap.reduce(
      (sum, milestone) => sum + milestone.subtasks.filter(subtask => subtask.isCompleted).length,
      0
    );
    ...
    const updateData: Partial<UserGoal> = {
      ...
      statistics: {
        ...goalData.statistics,
        timeSpent: goalData.statistics.timeSpent + (request.timeSpent || 0)
      }
    };
...
export async function logGoalSession(
  userId: string,
  goalId: string,
  milestoneId: string,
  duration: number,
...
export async function submitGoalToMarketplace(
  userId: string,
  request: SubmitToMarketplaceRequest
): Promise<string> {
  try {
    ...
    const marketplaceGoal: MarketplaceGoal = {
    ...
    const moderationItem: AdminModerationQueue = {
      id: queueId,
...
export async function moderateGoal(
  queueId: string,
  goalId: string,
  adminId: string,
...
export async function generateDailyAnalytics(): Promise<void> {
  try {
    ...
    const [
      totalUsersSnapshot,
      activeUsersSnapshot,
      goalsCreatedSnapshot,
      goalsCompletedSnapshot,
      marketplaceSubmissionsSnapshot,
      marketplaceApprovalsSnapshot
    ] = await Promise.all([
