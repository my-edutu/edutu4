 ...
 */
builderRouter.get('/templates',
  asyncHandler(async (req: Request, res: Response) => {
    const userId = req.user!.uid;
    
    logger.info('CV templates requested', { userId });
...
builderRouter.post('/start',
  asyncHandler(async (req: Request, res: Response) => {
    ...
    logger.info('CV building started', { userId, templateId });
 ...
 */
builderRouter.put('/:sessionId/section/:sectionName',
  asyncHandler(async (req: Request, res: Response) => {
    const userId = req.user!.uid;
    const { sessionId, sectionName } = req.params;
    const sectionData = req.body;

    logger.info('CV section update', { userId, sessionId, sectionName });
 ...
 */
builderRouter.post('/suggestions/:sectionName',
  asyncHandler(async (req: Request, res: Response) => {
    const userId = req.user!.uid;
    const { sectionName } = req.params;
    const { currentContent, jobDescription, industry } = req.body;

    logger.info('AI suggestions requested', { userId, sectionName });
    ...
    res.json({
      success: true,
      data: {
        suggestions: suggestions[sectionName as keyof typeof suggestions] || [],
        sectionName,
 ...
 */
builderRouter.post('/:sessionId/preview',
  asyncHandler(async (req: Request, res: Response) => {
    const userId = req.user!.uid;
    const { sessionId } = req.params;
    const { format = 'html' } = req.body;

    logger.info('CV preview requested', { userId, sessionId, format });
 ...
 */
builderRouter.post('/:sessionId/generate',
  asyncHandler(async (req: Request, res: Response) => {
    const userId = req.user!.uid;
    const { sessionId } = req.params;
    const { format = 'pdf', fileName } = req.body;

    logger.info('CV generation requested', { userId, sessionId, format });
 ...
 */
builderRouter.get('/:sessionId',
  asyncHandler(async (req: Request, res: Response) => {
    const userId = req.user!.uid;
    const { sessionId } = req.params;

    logger.info('Build session requested', { userId, sessionId });
 ...
 */
builderRouter.delete('/:sessionId',
  asyncHandler(async (req: Request, res: Response) => {
    const userId = req.user!.uid;
    const { sessionId } = req.params;

    logger.info('Build session deletion requested', { userId, sessionId });
